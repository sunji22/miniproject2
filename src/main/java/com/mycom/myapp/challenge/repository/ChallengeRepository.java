...
public interface ChallengeRepository extends JpaRepository<Challenge, Long> {
	...
	Page<Challenge> findByStatusNot(ChallengeStatus status, Pageable pageable);
	
	// Title 필터링 -> 생성일 순 정렬
	List<Challenge> findByTitleOrderByCreatedAt(ChallengeStatus status, Pageable pageable);
	
	// status가 DELETED가 아닌 것만 조회
    Optional<Challenge> findByIdAndStatusNot(Long id, ChallengeStatus status);
